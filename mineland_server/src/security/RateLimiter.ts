...
export class RateLimiter {
  ...
  private checkLimit(
    ...
    // Update history to only keep recent actions (for cleanup)
    if (actionType === 'any') {
      const validActions = history.filter(r => r.timestamp >= windowStart);
      history.splice(0, history.length, ...validActions);
    }
