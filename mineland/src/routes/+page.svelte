<script>
	...
	onMount(function onMountHandler() {
		...
		welcomeElement = document.getElementById('welcome-screen');
	...
	function getClickedTile(event) {
		if (!currentPlayer || !canvasRect) return { x: null, y: null };
	...
	function getHoveredTile(mouseX, mouseY) {
		if (!currentPlayer) return { x: null, y: null };
	...
	function updateUI() {
		if (!uiNeedsUpdate) return; // Skip if no UI changes needed
		...
		if (lastUIValues.flags !== currentPlayer.flags) {
			if (playerFlagsEl) playerFlagsEl.textContent = currentPlayer.flags;
		...
		if (lastUIValues.mines !== gameState.gameInfo.minesRemaining) {
			if (minesRemainingEl) minesRemainingEl.textContent = gameState.gameInfo.minesRemaining;
	...
	function updateLeaderboard() {
		if (!leaderboardElement || !leaderboardPlayers) return;
		...
		if (leaderboardHash === lastLeaderboardHash) return;
	...
	function renderNumber(x, y, number) {
		...
		ctx.fillText(number.toString(), x + Math.floor(TILE_SIZE / 2), y + Math.floor(TILE_SIZE / 2));
	}
	...
	function renderFlag(x, y, isCorrect = false) {
		...
		ctx.lineWidth = 2;
		ctx.beginPath();
		ctx.moveTo(flagX - Math.floor(flagWidth / 2), flagY - flagHeight);
		ctx.lineTo(flagX - Math.floor(flagWidth / 2), flagY + flagHeight);
		ctx.stroke();
		...
		ctx.beginPath();
		ctx.moveTo(flagX - Math.floor(flagWidth / 2), flagY - flagHeight);
		ctx.lineTo(flagX + Math.floor(flagWidth / 2), flagY - Math.floor(flagHeight / 2));
		ctx.lineTo(flagX - Math.floor(flagWidth / 2), flagY);
		ctx.closePath();
		ctx.fill();
